 ...
 * Edited by: Robin Olejnik 05.02 2013
...
#define JUMPER_BIT PB3          /* bit for button input/output */

/* ------------------------------------------------------------------------- */

static unsigned char    reportBuffer[2];    /* buffer for HID reports */
static unsigned char    idleRate;           /* in 4 ms units */
static unsigned char    reportCount;		/* current report */
unsigned char key = 0;
static unsigned char    buttonState;		/*  stores state of button */
static unsigned char	debounceTimeIsOver;	/* for switch debouncing */
...
static void checkButtonChange(void) {
	...
	unsigned char tempButtonValue = bit_is_clear(BUTTON_PIN, BUTTON_BIT); //status of switch is stored in tempButtonValue 
